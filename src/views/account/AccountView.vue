<template>
  <div class="max-w-[1100px]">
    ...
    <!-- ══════════ OVERVIEW ══════════ -->
    <div v-if="activeTab === 'overview'" class="grid grid-cols-1 lg:grid-cols-[300px_1fr] gap-6">
      <!-- Identity -->
      <div class="bg-white rounded-2xl border border-gray-200 p-6 flex flex-col items-start">
        <div class="w-24 h-24 rounded-full overflow-hidden ring-4 ring-ditto-light-grey">
          <img :src="profile.avatar" alt="" class="w-full h-full object-cover" />
        </div>
        <p class="font-satoshi font-black text-2xl tracking-[-0.02em] text-ditto-text mt-4">{{ profile.name }}</p>
        <p class="text-sm text-ditto-subtext mt-0.5 flex items-center gap-1.5">
          {{ profile.email }}
        </p>
        ...
        <div class="w-full border-t border-gray-100 mt-6 pt-5 space-y-3 text-sm">
        ...
        <button @click="activeTab = 'settings'" class="mt-6 w-full px-5 py-2.5 rounded-full border border-gray-200 text-sm font-semibold text-ditto-text hover:border-ditto-purple hover:text-ditto-purple transition-colors">Edit profile</button>
      </div>
      ...
      <div class="space-y-6">
        ...
        <div class="bg-white rounded-2xl border border-gray-200 p-6 sm:p-7">
          <div class="flex items-start justify-between gap-4 mb-5">
            <h3 class="font-satoshi font-black tracking-[-0.03em] text-lg text-ditto-text">Your plan</h3>
          ...
          <div class="grid grid-cols-1 md:grid-cols-[220px_1fr] gap-6">
            <div class="relative overflow-hidden rounded-2xl p-5 text-white plan-tile min-h-[120px] flex flex-col justify-end">
              ...
              <p class="relative font-satoshi font-black text-2xl tracking-[-0.02em]">{{ billing.planName }}</p>
              <p class="relative text-xs text-white/70">{{ profile.currency.symbol }}{{ billing.amountDue }}/year</p>
            </div>
            <div>
              <p class="text-sm text-ditto-text leading-relaxed">{{ billing.planBlurb }}</p>
        ...
        <!-- Billing -->
        <div class="bg-white rounded-2xl border border-gray-200 p-6 sm:p-7">
          <h3 class="font-satoshi font-black tracking-[-0.03em] text-lg text-ditto-text mb-5">Billing &amp; payments</h3>
          <div class="grid grid-cols-1 sm:grid-cols-3 gap-6">
            <div>
              <p class="text-xs text-ditto-subtext mb-1">Next payment</p>
              <p class="font-satoshi font-black text-2xl tracking-[-0.02em] text-ditto-text">{{ profile.currency.symbol }}{{ billing.amountDue }}<span class="text-sm font-medium text-ditto-subtext">.00</span></p>
              <p class="text-sm text-ditto-subtext">Due {{ billing.renewsOn }}</p>
            </div>
            <div>
              <p class="text-xs text-ditto-subtext mb-1">Payment method</p>
              <p class="text-sm font-semibold text-ditto-text flex items-center gap-2">
              ...
              <p class="text-sm text-ditto-subtext">Expires {{ billing.paymentMethod.expires }}</p>
            </div>
            <div>
              <p class="text-xs text-ditto-subtext mb-1">Last payment</p>
              <p class="text-sm font-semibold text-ditto-text">{{ profile.currency.symbol }}{{ billing.lastPayment.amount }} · {{ billing.lastPayment.date }}</p>
              <p class="text-sm text-ditto-subtext">{{ billing.lastPayment.description }}</p>
            </div>
          </div>
    ...
    <!-- ══════════ SECURITY ══════════ -->
    <div v-else-if="activeTab === 'security'" class="grid grid-cols-1 lg:grid-cols-2 gap-6 items-start">
      <!-- Password -->
      <div class="bg-white rounded-2xl border border-gray-200 p-6 sm:p-7">
        <div class="flex items-start justify-between gap-4 mb-1">
          <h3 class="font-satoshi font-black tracking-[-0.03em] text-lg text-ditto-text">Password</h3>
          <span class="text-xs text-ditto-subtext">Last changed {{ security.passwordChangedOn }}</span>
        </div>
        <p class="text-sm text-ditto-subtext mb-5">Change it here, or we can email you a reset link if you've forgotten it.</p>
        <div class="space-y-4">
          <div>
            <label class="block text-xs font-medium text-ditto-subtext mb-1">Current password</label>
            <input v-model="pw.current" type="password" placeholder="••••••••••••" :class="fieldClass" />
          </div>
          <div>
            <label class="block text-xs font-medium text-ditto-subtext mb-1">New password</label>
          ...
          <div>
            <label class="block text-xs font-medium text-ditto-subtext mb-1">Confirm new password</label>
      ...
      <div class="space-y-6">
        ...
        <div class="bg-white rounded-2xl border border-gray-200 p-6 sm:p-7">
          <div class="flex items-start justify-between gap-4">
            <div>
              <h3 class="font-satoshi font-black tracking-[-0.03em] text-lg text-ditto-text flex items-center gap-2">
              ...
              <p class="text-sm text-ditto-subtext mt-1 max-w-md">Protect your royalties with a code from an authenticator app whenever you sign in on a new device.</p>
            </div>
        ...
        <!-- Security question -->
        <div class="bg-white rounded-2xl border border-gray-200 p-6 sm:p-7">
          <h3 class="font-satoshi font-black tracking-[-0.03em] text-lg text-ditto-text">Security question</h3>
          <p class="text-sm text-ditto-subtext mt-1 mb-4">We'll ask this when we need to confirm it's you, like on a payout change.</p>
          ...
          <div v-else class="space-y-3">
            ...
            <div class="flex items-center gap-3">
              ...
              <button v-if="security.securityQuestionSet" @click="editingQuestion = false" class="text-sm text-ditto-subtext hover:text-ditto-text">Cancel</button>
            </div>
          </div>
        </div>
        ...
        <div class="bg-white rounded-2xl border border-gray-200 p-6 sm:p-7">
          <div class="flex items-center justify-between mb-4">
            <h3 class="font-satoshi font-black tracking-[-0.03em] text-lg text-ditto-text">Where you're signed in</h3>
            <button @click="signOutOthers" class="text-xs font-medium text-error hover:underline">Sign out everywhere else</button>
          </div>
          <div class="divide-y divide-gray-100">
            <div v-for="s in security.sessions" :key="s.id" class="flex items-center gap-3 py-3">
              ...
              <div class="flex-1 min-w-0">
                <p class="text-sm font-medium text-ditto-text">{{ s.device }} <span v-if="s.current" class="ml-1 text-[10px] font-semibold px-1.5 py-0.5 rounded bg-success/15 text-success">This device</span></p>
    ...
    <div v-else class="grid grid-cols-1 lg:grid-cols-2 gap-6 items-start">
      <!-- Profile -->
      <div class="bg-white rounded-2xl border border-gray-200 p-6 sm:p-7">
        <h3 class="font-satoshi font-black tracking-[-0.03em] text-lg text-ditto-text mb-5">Profile</h3>
        ...
        <div class="space-y-5 pt-5">
          <div>
            <label class="block text-xs font-medium text-ditto-subtext mb-1">Name</label>
            <input v-model="draft.name" type="text" :class="fieldClass" />
          </div>
          <div>
            <label class="block text-xs font-medium text-ditto-subtext mb-1">Email</label>
          ...
          <div class="grid grid-cols-2 gap-4">
            <div>
              <label class="block text-xs font-medium text-ditto-subtext mb-1">Currency</label>
            ...
            <div>
              <label class="block text-xs font-medium text-ditto-subtext mb-1">Country</label>
      ...
      <div class="space-y-6">
        ...
        <div class="bg-white rounded-2xl border border-gray-200 p-6 sm:p-7">
          <h3 class="font-satoshi font-black tracking-[-0.03em] text-lg text-ditto-text mb-1">Email preferences</h3>
          <p class="text-sm text-ditto-subtext mb-4">Account and payout emails always come through — these are the optional ones.</p>
          <div class="divide-y divide-gray-100">
            <label class="flex items-center justify-between gap-4 py-3 cursor-pointer">
              <span><span class="block text-sm font-medium text-ditto-text">Royalty statements</span><span class="block text-xs text-ditto-subtext">When a new statement lands</span></span>
              <Toggle v-model="prefs.emailRoyalties" />
            </label>
            <label class="flex items-center justify-between gap-4 py-3 cursor-pointer">
              <span><span class="block text-sm font-medium text-ditto-text">Release updates</span><span class="block text-xs text-ditto-subtext">Store approvals, rejections and go-lives</span></span>
              <Toggle v-model="prefs.emailReleases" />
            </label>
            <label class="flex items-center justify-between gap-4 py-3 cursor-pointer">
              <span><span class="block text-sm font-medium text-ditto-text">News &amp; offers</span><span class="block text-xs text-ditto-subtext">New tools, perks and promotions</span></span>
        ...
        <div class="bg-white rounded-2xl border border-gray-200 p-6 sm:p-7">
          <div class="flex items-start justify-between gap-4">
            <div>
              <h3 class="font-satoshi font-black tracking-[-0.03em] text-lg text-ditto-text">AI music training</h3>
              <p class="text-sm text-ditto-subtext mt-1 max-w-md">License your back catalogue and opt in new releases to train AI models, and get paid for it. You can opt out at any time.
        ...
        <!-- Danger zone (new) -->
        <div class="rounded-2xl border border-error/30 p-6 sm:p-7">
          <h3 class="font-satoshi font-black tracking-[-0.03em] text-lg text-ditto-text">Close your account</h3>
          <p class="text-sm text-ditto-subtext mt-1 mb-4">Releases stay live under Release Protection; anything else comes down from stores. Unpaid royalties are paid out first.</p>
...
const fieldClass = 'w-full px-0 py-2.5 border-0 border-b border-gray-300 text-sm text-ditto-text bg-transparent placeholder:text-ditto-subtext focus:outline-none focus:border-ditto-purple transition-colors'
